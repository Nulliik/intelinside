...
export default function Runtimes() {
  ...
  const newest = customRuntimes.data?.items.slice(0, 6) ?? []

  const register = user ? (
    <Button variant="outline" render={<Link to="/runtimes/custom/new" />} nativeButton={false}>
      <Plus data-icon="inline-start" /> Register a custom runtime
    </Button>
  ) : (
    <Button variant="outline" onClick={() => requestSignIn('/runtimes/custom/new')}>
      <Plus data-icon="inline-start" /> Register a custom runtime
    </Button>
  )
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Each runtime is seeded from the catalog. Anyone can register a custom runtime on top of one — a fork, a patch, a custom kernel — and post results against it. Boards rank stock runs against each other; custom runtimes rank among themselves."
        actions={register}
      />
      ...
      <Section label="Newest custom runtimes">
        {newest.length ? (
          <CellGrid cols={3}>
            {newest.map((b) => (
              <Cell key={b.id}>
          ...
          <EmptyState
            title="No custom runtimes yet."
            description="A custom runtime is one somebody changed — a custom kernel or op, a patch, a fork. Register yours and post the numbers it gets."
            action={register}
          />
        )}
      </Section>
