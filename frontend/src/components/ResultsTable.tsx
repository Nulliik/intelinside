        ...
        <TableBody>
          {results.map((r) => (
            <TableRow key={r.id} className="cursor-pointer" onClick={() => navigate(`/results/${r.id}`)}>
              ...
              <TableCell className={td}>
                <span className="inline-flex items-center gap-1 whitespace-nowrap"><RuntimeBadge runtime={rt[r.runtimeId]} version={r.runtimeVersion} />{r.customRuntime ? <BuildLink build={r.customRuntime} className="text-xs" /> : <ExecutionBadge result={r} iconOnly />}</span>
              </TableCell>
